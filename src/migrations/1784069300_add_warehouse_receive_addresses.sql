...
-- (the migration runner may re-run this after a partial/manual apply).
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'warehouse_receive_addresses_id_wh_uniq') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'shipments_inbound_items_receive_addr_fk') THEN
    ALTER TABLE shipments_inbound_items
