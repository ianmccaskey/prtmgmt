  ...
  return action('getWarehouseThroughputSummary', 'SQL', {
    ...
    query: `
      SELECT
        ...
        SUM(soi.total_qty) AS total_kits_shipped,
        ...
        CASE WHEN COUNT(DISTINCT DATE_TRUNC('month', so.shipped_date)) > 0
          THEN SUM(soi.total_qty)::numeric / COUNT(DISTINCT DATE_TRUNC('month', so.shipped_date))
          ELSE 0 END AS avg_kits_per_month,
        COALESCE(SUM(so.internal_shipping_cost_usd), 0) AS total_shipping_cost,
        CASE WHEN SUM(soi.total_qty) > 0
          THEN COALESCE(SUM(so.internal_shipping_cost_usd), 0) / SUM(soi.total_qty)
